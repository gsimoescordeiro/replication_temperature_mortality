...
#
# Table A1 as submitted uses a different unit again - the region-day series -
# which is why its standard deviations are an order of magnitude smaller than
# the municipality-day ones.  Section 4 reproduces it exactly and the header
# there explains the difference.
#
...
# ------------------------------------------------------------------------------
# Table A1 as submitted is built on a different unit of analysis from section 1
# above, which is why its standard deviations are so much smaller.  Section 1
# describes MUNICIPALITY-DAYS: how much rates vary across places and days.
...
# GDP per capita uses total population, as in the original table (the Appendix D
# split in 06_gdp_split.R uses the 20+ denominator instead, so the two are not
# directly comparable and are not meant to be).
gdp_pc_lookup <- NULL
