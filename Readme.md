...
- **The 50+ figures combine two separately fitted models**, 50-69 and 70+. Each is scaled by its
  own population (`pop_50_69`, `pop_70_plus`), so the two are counts of deaths among different
  people and the point estimates add exactly. `07` writes the sensitivity of the combined interval
  alongside each estimate; the appendix documents the aggregation.
